...
contract WolfPack is Ownable, ERC721, IERC2981 {
    ...
    constructor(address[] memory _whitelisted, address[] memory _airDrop)
        ERC721("The Wolf Pack", "WPTK")
    {
        _tokenIds.increment();
        mintingSupply = 5405 - preMinted;
        _isPreRelease = false;
        saleIsActive = false;
    ...
    function setMinters(address[] memory _minters, uint256[] memory _tokenId)
        external
        onlyOwner
    {
        require(
            _minters.length == _tokenId.length,
            "Array length missmatch! Please verify inputs"
        );
